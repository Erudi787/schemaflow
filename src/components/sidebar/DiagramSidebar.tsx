...
export function DiagramSidebar({ isOpen, onClose }: DiagramSidebarProps) {
    const {
        savedDiagrams,
        activeDiagramId,
        schemaModel,
    ...
    return (
        <>
            ...
            <aside
                className={`
                    fixed inset-y-0 left-0 z-50 w-64 shrink-0 border-r border-border bg-bg-secondary flex flex-col transition-transform duration-300 ease-in-out
            ...
            >
                ...
                <div className="p-4 border-b border-border flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-[10px] text-text-muted mt-1">Interactive Schema</p>
                    </div>
                    {/* Close button for mobile */}
                    <button
                        onClick={onClose}
                        className="md:hidden p-1.5 text-text-muted hover:text-text-primary hover:bg-bg-tertiary rounded transition-colors"
                    >
                        <X size={16} />
                    </button>
                </div>
                        ...
                        <div className="p-1.5 flex flex-col gap-0.5">
                            {savedDiagrams.map((diagram) => (
                                <div
                                    ...
                                    onClick={() => {
                                        if (editingId !== diagram.id) {
                                            loadDiagram(diagram.id);
                                            // Auto-close sidebar on mobile after selecting a diagram
